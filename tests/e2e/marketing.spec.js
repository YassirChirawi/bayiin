...
async function login(page) {
    ...
    await page.waitForURL(/.*\/(dashboard|settings|orders|products|marketing)/, { timeout: 30000, waitUntil: 'commit' });
}
...
test.describe('Marketing Module E2E', () => {
    ...
    test('Marketing Dashboard and Segmentation', async ({ page }) => {
        ...
        await expect(page.locator('h1, h2, h3, div').filter({ hasText: /Marketing|Campagnes/i }).first()).toBeVisible({ timeout: 15000 });
    });
});
