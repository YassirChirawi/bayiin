...
async function login(page) {
    ...
    await page.waitForURL(/.*\/(dashboard|settings|orders|products|automations)/, { timeout: 30000 });
}
...
test.describe('Automations Module E2E', () => {
    ...
    test('Create a simple automation workflow', async ({ page }) => {
        ...
        await expect(page.locator('h1, h2, h3, button').filter({ hasText: /Automatisations|Automation|Nouveau|New/i }).first()).toBeVisible({ timeout: 15000 });
    });
});
