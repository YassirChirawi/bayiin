...
async function login(page) {
    ...
    await page.waitForURL(/.*\/(dashboard|settings|orders|products|customers)/, { timeout: 30000 });
}
...
test.describe('Customers Module E2E', () => {
    ...
    test('Customer creation and automatic lookup during order', async ({ page }) => {
        ...
        await expect(page.locator('h1, h2, h3, div').filter({ hasText: /Clients|Customers/i }).first()).toBeVisible({ timeout: 15000 });
    });
});
